#!/usr/bin/env python3
"""
Delta Arc Corp S3 Integration Script - FINAL WORKING VERSION with .env support
...
from pyatlan.model.search import IndexSearchRequest, DSL, MatchAll, Term
...
class AtlanS3Integrator:
    ...
    def __init__(self, base_url: str, api_token: str, initials: str = "ARY"):
        ...
        self.client = AtlanClient(base_url=base_url, api_key=api_token)
    ...
    def create_s3_connection(self):
        ...
        try:
            ...
            dsl = DSL(query=Term(field="connectorType", value="s3"), size=10)
            request = IndexSearchRequest(dsl=dsl)

            response = self.client.asset.search(request)

            if hasattr(response, 'assets') and response.assets and len(response.assets) > 0:
                logger.info(f"✅ Found existing S3 connection: {response.assets[0].qualified_name}")
                return response.assets[0]
            elif hasattr(response, 'results') and response.results and len(response.results) > 0:
                logger.info(f"✅ Found existing S3 connection: {response.results[0].qualified_name}")
    ...
    def register_s3_bucket(self, connection):
        ...
        try:
            ...
            # Add compliance and security tags (removed for now - needs proper dict format)

            # Custom metadata for security (not supported by this S3Bucket model)
...
"""
                ...
                # Apply security and compliance tags (removed for now - needs proper dict format)

                # Custom metadata for governance (not supported by this S3Object model)
    ...
    def run_integration(self):
        ...
        try:
            ...
            logger.info(f"🆔 Bucket ARN: {self.bucket_arn}")
...
def main():
    ...
    # Configuration - .env file loaded automatically
    ...
    if not ATLAN_API_TOKEN:
        logger.error("❌ ATLAN_API_TOKEN not found in .env file")
        logger.info("💡 Check your .env file contains: ATLAN_API_TOKEN=your-token-here")
        return
    ...
    logger.info(f"📁 Loaded .env from current directory")
    ...
    if success:
        ...
        print("   1. Verify your API token is correct in .env")
